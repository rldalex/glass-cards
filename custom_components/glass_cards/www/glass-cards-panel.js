    ...
    `}_renderBreadcrumb(){if("rooms"===this._nav.section&&this._nav.roomId){const e=this.hass?.areas?.[this._nav.roomId];return N`
      ...
      <div class="page-wrap">
        <div class="page-header">
          ...
          <span class="page-subtitle">${ei("config.brand")} <span class="page-version">v${"0.0.231"}</span></span>
        </div>
  ...
  `]}getTrackedEntityIds(){const e=this._getEntityId(),t=e?[e]:[];return this._spotifyConfig?.entity_id&&!t.includes(this._spotifyConfig.entity_id)&&t.push(this._spotifyConfig.entity_id),t}_isNowPlaying(e){const t=this._spotifyConfig?.entity_id;if(!t)return!1;const i=this.hass?.states[t];return!(!i||"playing"!==i.state)&&(i.attributes.media_content_id??"")===e}_getPlaybackEntity(){const e=this._getEntityId();if(!e)return null;const t=this.hass?.states[e];if(!t)return null;if("playing"!==t.state&&"paused"!==t.state)return null;const i=t.attributes.media_position_updated_at,a=i?Date.parse(i):NaN;return{entityId:e,state:t.state,title:t.attributes.media_title??null,artist:t.attributes.media_artist??null,art:t.attributes.entity_picture??null,position:t.attributes.media_position??null,duration:t.attributes.media_duration??null,positionUpdatedAt:Number.isFinite(a)?a:null}}_focusSearchInput(){requestAnimationFrame(()=>{const e=this.renderRoot.querySelector("input.search-input");e?.focus()})}_getEntityId(){if(this._config?.entity)return this._config.entity;if(this._spotifyConfig.entity_id)return this._spotifyConfig.entity_id;if(this.hass){const e=Object.keys(this.hass.states).find(e=>e.startsWith("media_player.spotify"));if(e)return e}return""}shouldUpdate(e){return!!super.shouldUpdate(e)&&("speaker_picker"!==this._view||1!==e.size||!e.has("hass"))}connectedCallback(){super.connectedCallback(),this._listen("spotify-config-changed",()=>{this._configLoaded=!1,this._loadConfig()})}disconnectedCallback(){super.disconnectedCallback(),this._debounceTimer&&clearTimeout(this._debounceTimer),this._progressTimer&&(clearInterval(this._progressTimer),this._progressTimer=0),this._backend=void 0,this._configLoaded=!1,this._configLoadingInProgress=!1,window.removeEventListener("keydown",this._onPickerKeydown)}_collapseExpanded(){"speaker_picker"!==this._view?(this._foldOpen&&(this._foldOpen=!1),this._drilldown&&(this._drilldown=null,this._view=this._searchQuery?"search":"library")):this._closePicker()}updated(e){super.updated(e),e.has("hass")&&this.hass&&(this._backend&&this._backend.connection!==this.hass.connection&&(this._backend=void 0,this._configLoaded=!1),this._configLoaded||this._configLoadingInProgress||(this._backend=new gi(this.hass),this._loadConfig()),this._syncProgressTimer())}_syncProgressTimer(){const e=this._getPlaybackEntity(),t="playing"===e?.state&&null!=e.duration&&e.duration>0&&null!=e.positionUpdatedAt;t&&!this._progressTimer?this._progressTimer=window.setInterval(()=>this.requestUpdate(),1e3):!t&&this._progressTimer&&(clearInterval(this._progressTimer),this._progressTimer=0)}async _loadConfig(){if(!this._backend||this._configLoadingInProgress)return;this._configLoadingInProgress=!0;const e=++this._loadVersion;try{const t=await this._backend.send("get_config");if(e!==this._loadVersion)return;if(t?.spotify_card&&(this._spotifyConfig=t.spotify_card),this._configLoaded=!0,await this._checkSpotifyStatus(),e!==this._loadVersion)return;this._spotifyConfigured&&this._loadLibrary(),this.requestUpdate()}catch{}finally{e===this._loadVersion&&(this._configLoadingInProgress=!1)}}async _checkSpotifyStatus(){if(this._backend)try{const e=await this._backend.send("spotify_status");this._spotifyConfigured=e?.configured??!1}catch{this._spotifyConfigured=!1}}async _loadLibrary(){if(!this._backend)return;this._libraryLoading=!0,this._error=null;const e=this._spotifyConfig.max_items_per_section;try{const[t,i,a,s]=await Promise.all([this._backend.send("spotify_browse",{category:"playlists",limit:e,offset:0,sort_order:this._spotifyConfig.sort_order}),this._backend.send("spotify_browse",{category:"recently_played",limit:e,offset:0,sort_order:this._spotifyConfig.sort_order}),this._backend.send("spotify_browse",{category:"saved_tracks",limit:e,offset:0,sort_order:this._spotifyConfig.sort_order}),this._backend.send("spotify_browse",{category:"saved_shows",limit:e,offset:0,sort_order:this._spotifyConfig.sort_order})]);this._playlists=(t?.items??[]).filter(Boolean),this._recentlyPlayed=(i?.items??[]).filter(Boolean),this._savedTracks=(a?.items??[]).filter(Boolean),this._savedShows=(s?.items??[]).filter(Boolean).map(e=>e.show??e),this._sectionTotals={playlists:t?.total??0,recently_played:i?.total??0,saved_tracks:a?.total??0,saved_shows:s?.total??0};const r=[];for(const e of this._recentlyPlayed){const t=e.track??e;!t.id||"track"!==t.type&&t.type||r.push(t.id)}for(const e of this._savedTracks){const t=e.track??e;t.id&&r.push(t.id)}r.length&&this._checkSavedStatus(r)}catch(t){this._handleApiError(t)}finally{this._libraryLoading=!1}}_onSearchInput(e){const t=e.target.value;if(this._searchQuery=t,this._debounceTimer&&clearTimeout(this._debounceTimer),0===t.length)return this._view="library",this._searchResults={tracks:[],playlists:[],shows:[]},void(this._searchOffset=0);this._foldOpen||(this._foldOpen=!0),this._view="search",this._debounceTimer=window.setTimeout(()=>this._doSearch(!1),300)}_clearSearch(){this._searchQuery="",this._view="library",this._searchResults={tracks:[],playlists:[],shows:[]},this._searchOffset=0,this._foldOpen=!1}async _doSearch(e){if(!this._backend||!this._searchQuery)return;const t=++this._searchVersion;this._searchLoading=!0,this._error=null;const i=e?this._searchOffset:0;try{let a;a="tracks"===this._tab?["track"]:"playlists"===this._tab?["playlist"]:"podcasts"===this._tab?["show"]:["track","playlist","show"];const s=await this._backend.send("spotify_search",{query:this._searchQuery,types:a,limit:12,offset:i});if(t!==this._searchVersion)return;const r=(s?.tracks?.items??[]).filter(Boolean),o=(s?.playlists?.items??[]).filter(Boolean),n=(s?.shows?.items??[]).filter(Boolean);this._searchResults=e?{tracks:[...this._searchResults.tracks,...r],playlists:[...this._searchResults.playlists,...o],shows:[...this._searchResults.shows,...n]}:{tracks:r,playlists:o,shows:n},this._searchOffset=i+12;const c=(s?.tracks?.total??0)+(s?.playlists?.total??0)+(s?.shows?.total??0),l=this._searchResults.tracks.length+this._searchResults.playlists.length+this._searchResults.shows.length;this._searchHasMore=l<c;const d=r.filter(e=>e.id).map(e=>e.id);d.length&&this._checkSavedStatus(d)}catch(a){if(t!==this._searchVersion)return;this._handleApiError(a)}finally{t===this._searchVersion&&(this._searchLoading=!1)}}async _openDrilldown(e,t,i,a,s){if(this._backend){this._view="drilldown",this._drilldown={title:i,type:e,id:t,image:a,subtitle:s,items:[],total:0,offset:0,loading:!0},this._error=null;try{const i="playlist"===e?"playlist_tracks":"album_tracks",a=await this._backend.send("spotify_browse",{category:i,content_id:t,limit:20,offset:0,sort_order:this._spotifyConfig.sort_order}),s=a?.items??[];if(!this._drilldown)return;this._drilldown={...this._drilldown,items:s,total:a?.total??0,offset:20,loading:!1};const r=s.map(e=>(e.track??e).id).filter(Boolean);r.length&&this._checkSavedStatus(r)}catch(r){this._handleApiError(r),this._drilldown&&(this._drilldown={...this._drilldown,loading:!1})}}}async _loadMoreDrilldown(){if(this._drilldown&&this._backend){this._drilldown={...this._drilldown,loading:!0};try{const e="playlist"===this._drilldown.type?"playlist_tracks":"album_tracks",t=await this._backend.send("spotify_browse",{category:e,content_id:this._drilldown.id,limit:20,offset:this._drilldown.offset,sort_order:this._spotifyConfig.sort_order}),i=t?.items??[];this._drilldown={...this._drilldown,items:[...this._drilldown.items,...i],offset:this._drilldown.offset+20,loading:!1};const a=i.map(e=>(e.track??e).id).filter(Boolean);a.length&&this._checkSavedStatus(a)}catch(e){this._handleApiError(e),this._drilldown&&(this._drilldown={...this._drilldown,loading:!1})}}}_goBack(){this._drilldown=null,this._view=this._searchQuery?"search":"library"}_openPicker(e){if(this._pickerItem=e,this._view="speaker_picker",this._selectedSpeakers=new Set,window.addEventListener("keydown",this._onPickerKeydown),queueMicrotask(()=>{requestAnimationFrame(()=>{const e=this.renderRoot.querySelector(".picker-close");e?.focus()})}),this.hass){const e=this._spotifyConfig.visible_speakers,t=e.length>0;this._speakers=Object.entries(this.hass.states).filter(([i])=>!!i.startsWith("media_player.")&&!(t&&!e.includes(i))).map(([e,t])=>{const i=t.attributes.device_class??"";let a="mdi:speaker";return"tv"===i||e.includes("tv")?a="mdi:television":"receiver"===i?a="mdi:audio-video":(e.includes("nest")||e.includes("hub")||e.includes("echo_show"))&&(a="mdi:tablet"),{entityId:e,name:t.attributes.friendly_name??e,state:t.state,mediaTitle:t.attributes.media_title??null,icon:a}}).sort((i,a)=>{if(t)return e.indexOf(i.entityId)-e.indexOf(a.entityId);const s=e=>"playing"===e?0:"paused"===e?1:2;return s(i.state)-s(a.state)})}}_closePicker(){this._pickerItem=null,this._view=this._drilldown?"drilldown":this._searchQuery?"search":"library",window.removeEventListener("keydown",this._onPickerKeydown)}_toggleSpeakerSelection(e){const t=new Set(this._selectedSpeakers);t.has(e)?t.delete(e):t.add(e),this._selectedSpeakers=t}async _playOnSelectedSpeakers(){if(!this.hass||!this._pickerItem||0===this._selectedSpeakers.size)return;li(this,"light");const e=this._pickerItem,t=e.uri??`spotify:${e.type}:${e.id}`,i=[...this._selectedSpeakers],a="track"===e.type?"music":"playlist"===e.type?"playlist":"album"===e.type?"music":"podcast";try{for(const e of i){const t=this.hass.states[e];if(!t)continue;const i=t.attributes.group_members;i&&i.length>1&&this._safeCallService("media_player","unjoin",{},{entity_id:e})}i.length>1&&await new Promise(e=>setTimeout(e,600));const s=i[0];if(this._safeCallService("media_player","play_media",{media_content_id:t,media_content_type:a},{entity_id:s}),i.length>1){const e=i.slice(1),r=this.hass.states[s];if(r&&!!(524288&r.attributes.supported_features))await new Promise(e=>setTimeout(e,800)),this._safeCallService("media_player","join",{group_members:e},{entity_id:s});else for(const i of e)this._safeCallService("media_player","play_media",{media_content_id:t,media_content_type:a},{entity_id:i})}"track"!==e.type&&"episode"!==e.type||!this._backend||this._seedRadioQueue(e)}catch{}this._closePicker()}async _seedRadioQueue(e){if(!this._backend)return;const t=++this._radioQueueVersion;try{if(await new Promise(e=>setTimeout(e,2e3)),!this._backend||t!==this._radioQueueVersion)return;const i=await this._backend.send("spotify_browse",{category:"recommendations",seed_tracks:[e.id],limit:20});if(t!==this._radioQueueVersion)return;const a=i?.tracks??[];fe.emit("radio-queue-started",{count:a.length});let s=0;for(let e=0;e<a.length;e++){const i=a[e];if(!this._backend||t!==this._radioQueueVersion)break;const r=i.uri??`spotify:track:${i.id}`;try{await this._backend.send("spotify_add_to_queue",{uri:r}),s++,fe.emit("radio-queue-track-added",{track:{id:i.id,name:i.name,uri:r,artist:Or(i)||void 0},index:e}),await new Promise(e=>setTimeout(e,150))}catch{break}}t===this._radioQueueVersion&&fe.emit("radio-queue-complete",{total:s})}catch(i){t===this._radioQueueVersion&&fe.emit("radio-queue-error",{message:i.message??"Unknown error"})}}async _loadMoreItems(e){if(!this._backend||this._loadingMore[e])return;this._loadingMore={...this._loadingMore,[e]:!0};const t=this._spotifyConfig.max_items_per_section;let i=0;"playlists"===e?i=this._playlists.length:"recently_played"===e?i=this._recentlyPlayed.length:"saved_tracks"===e?i=this._savedTracks.length:"saved_shows"===e&&(i=this._savedShows.length);try{const a=await this._backend.send("spotify_browse",{category:e,limit:t,offset:i,sort_order:this._spotifyConfig.sort_order}),s=(a?.items??[]).filter(Boolean);if("playlists"===e)this._playlists=[...this._playlists,...s];else if("recently_played"===e)this._recentlyPlayed=[...this._recentlyPlayed,...s];else if("saved_tracks"===e){this._savedTracks=[...this._savedTracks,...s];const e=s.map(e=>(e.track??e).id).filter(Boolean);e.length&&this._checkSavedStatus(e)}else"saved_shows"===e&&(this._savedShows=[...this._savedShows,...s.map(e=>e.show??e)]);null!=a?.total&&(this._sectionTotals={...this._sectionTotals,[e]:a.total})}catch(a){this._handleApiError(a)}finally{this._loadingMore={...this._loadingMore,[e]:!1}}}_renderLoadMore(e,t){const i=this._sectionTotals[e]??0;if(t>=i)return W;const a=this._loadingMore[e],s=ei(Mr[e]);return N`
    ...
    `}}Ar([ue()],Fr.prototype,"_view"),Ar([ue()],Fr.prototype,"_tab"),Ar([ue()],Fr.prototype,"_searchQuery"),Ar([ue()],Fr.prototype,"_playlists"),Ar([ue()],Fr.prototype,"_recentlyPlayed"),Ar([ue()],Fr.prototype,"_savedTracks"),Ar([ue()],Fr.prototype,"_savedShows"),Ar([ue()],Fr.prototype,"_searchResults"),Ar([ue()],Fr.prototype,"_searchLoading"),Ar([ue()],Fr.prototype,"_searchOffset"),Ar([ue()],Fr.prototype,"_searchHasMore"),Ar([ue()],Fr.prototype,"_drilldown"),Ar([ue()],Fr.prototype,"_speakers"),Ar([ue()],Fr.prototype,"_pickerItem"),Ar([ue()],Fr.prototype,"_selectedSpeakers"),Ar([ue()],Fr.prototype,"_error"),Ar([ue()],Fr.prototype,"_libraryLoading"),Ar([ue()],Fr.prototype,"_spotifyConfigured"),Ar([ue()],Fr.prototype,"_foldOpen"),Ar([ue()],Fr.prototype,"_savedMap"),Ar([ue()],Fr.prototype,"_sectionTotals"),Ar([ue()],Fr.prototype,"_loadingMore");try{customElements.define("glass-spotify-card",Fr)}catch{}ri("glass-camera-carousel-card-editor");var qr=Object.defineProperty,Vr=(e,t,i,a)=>{for(var s,r=void 0,o=e.length-1;o>=0;o--)(s=e[o])&&(r=s(t,i,r)||r);return r&&qr(t,i,r),r};const Nr=1,Br="mdi:cctv",Ur="mdi:webcam",Wr="mdi:doorbell-video",Kr={person:"mdi:human",vehicle:"mdi:car",pet:"mdi:dog",animal:"mdi:paw",package:"mdi:package-variant",face:"mdi:face-recognition",baby_crying:"mdi:baby-face-outline",bicycle:"mdi:bicycle"},Yr={motion:/_(motion|mouvement|motion_detected)$/,record:/_(record|enregistrer|recordings)$/,siren:/^siren\./,floodlight:/_(floodlight|projecteur)$/,auto_tracking:/_(auto_tracking|suivi_automatique)$/,visitor:/_(visitor|visiteur|doorbell_chime)$/,privacy:/_(privacy_mode|mode_confidentialite)$/,detect:/_detect$/,snapshots:/_snapshots$/,is_dark:/_is_dark$/,battery:/_battery(_percentage|_level)?$/,battery_low:/_battery_low$/,sleep_status:/_sleep_status$/,ptz_up:/_ptz_up$/,ptz_down:/_ptz_down$/,ptz_left:/_ptz_left$/,ptz_right:/_ptz_right$/,ptz_zoom_in:/_ptz_zoom_(?:in|plus)$/,ptz_zoom_out:/_ptz_zoom_(?:out|minus)$/},Gr=[[/_person(ne)?(_detected|_occupancy)?$/,"person"],[/_vehicu?le(_detected|_occupancy)?$/,"vehicle"],[/_(pet|animal_domestique)(_detected|_occupancy)?$/,"pet"],[/_animal(_detected|_occupancy)?$/,"animal"],[/_(face|visage)(_detected|_occupancy)?$/,"face"],[/_(package|colis)(_detected|_occupancy)?$/,"package"],[/_(baby_crying|pleur_bebe)$/,"baby_crying"],[/_(bicycl?e|velo)(_detected|_occupancy)?$/,"bicycle"]];const Xr=new Map;function Qr(e){if(!e.entity_id.startsWith("event."))return!1;const t=e.attributes??{};if("doorbell"===t.device_class)return!0;const i=t.event_types;return!(!Array.isArray(i)||!i.includes("ring"))||!!/doorbell|sonnette|chime/i.test(e.entity_id)}function Jr(e,t,i){const a=i[e];if(!a?.device_id)return{motionSensorId:null,recordSwitchId:null,sirenId:null,floodlightId:null,autoTrackId:null,aiDetected:[],doorbellEventId:null,visitorSensorId:null,privacySwitchId:null,detectSwitchId:null,snapshotsSwitchId:null,isDarkSensorId:null,batterySensorId:null,batteryLowSensorId:null,sleepSensorId:null,ptzUpId:null,ptzDownId:null,ptzLeftId:null,ptzRightId:null,ptzZoomInId:null,ptzZoomOutId:null};const s=a.device_id;let r=s;for(const l of Object.keys(i)){if(i[l].device_id!==s)continue;const e=t[l];e&&((l.startsWith("binary_sensor.")||l.startsWith("event.")||l.startsWith("sensor.")&&Yr.battery.test(l))&&(r+=`:${l}=${e.state}`))}const o=Xr.get(e);if(o&&o.key===r)return o.result;const n=[];for(const[l,d]of Object.entries(i))d.device_id===s&&n.push(l);const c={motionSensorId:null,recordSwitchId:null,sirenId:null,floodlightId:null,autoTrackId:null,aiDetected:[],doorbellEventId:null,visitorSensorId:null,privacySwitchId:null,detectSwitchId:null,snapshotsSwitchId:null,isDarkSensorId:null,batterySensorId:null,batteryLowSensorId:null,sleepSensorId:null,ptzUpId:null,ptzDownId:null,ptzLeftId:null,ptzRightId:null,ptzZoomInId:null,ptzZoomOutId:null};for(const l of n){const e=t[l];if(e&&(l.startsWith("binary_sensor.")&&Yr.motion.test(l)&&(c.motionSensorId=l),l.startsWith("switch.")&&Yr.record.test(l)&&(c.recordSwitchId=l),Yr.siren.test(l)&&(c.sirenId=l),l.startsWith("light.")&&Yr.floodlight.test(l)&&(c.floodlightId=l),l.startsWith("switch.")&&Yr.auto_tracking.test(l)&&(c.autoTrackId=l),l.startsWith("switch.")&&Yr.privacy.test(l)&&(c.privacySwitchId=l),l.startsWith("binary_sensor.")&&Yr.visitor.test(l)&&(c.visitorSensorId=l),l.startsWith("switch.")&&Yr.detect.test(l)&&(c.detectSwitchId=l),l.startsWith("switch.")&&Yr.snapshots.test(l)&&(c.snapshotsSwitchId=l),l.startsWith("binary_sensor.")&&Yr.is_dark.test(l)&&(c.isDarkSensorId=l),l.startsWith("sensor.")&&Yr.battery.test(l)&&(c.batterySensorId=l),l.startsWith("binary_sensor.")&&Yr.battery_low.test(l)&&(c.batteryLowSensorId=l),l.startsWith("binary_sensor.")&&Yr.sleep_status.test(l)&&(c.sleepSensorId=l),l.startsWith("button.")&&(Yr.ptz_up.test(l)?c.ptzUpId=l:Yr.ptz_down.test(l)?c.ptzDownId=l:Yr.ptz_left.test(l)?c.ptzLeftId=l:Yr.ptz_right.test(l)?c.ptzRightId=l:Yr.ptz_zoom_in.test(l)?c.ptzZoomInId=l:Yr.ptz_zoom_out.test(l)&&(c.ptzZoomOutId=l)),Qr(e)&&(c.doorbellEventId=l),l.startsWith("binary_sensor.")&&"on"===e.state))for(const[t,i]of Gr)t.test(l)&&!c.aiDetected.includes(i)&&c.aiDetected.push(i)}return Xr.set(e,{key:r,result:c}),c}function Zr(e,t){const i=e.attributes?.icon;if(i)return i;if(t)return Wr;const a=e.entity_id;return a.includes("doorbell")||a.includes("sonnette")?Wr:a.includes("indoor")||a.includes("salon")||a.includes("chambre")?Ur:Br}class eo extends ci{constructor(){super(...arguments),this._carouselIndex=0,this._liveIds=new Set,this._foldOpen=!1,this.previewFullscreen=!1,this._camConfig=null,this._roomConfig=null,this._configLoaded=!1,this._configLoading=!1,this._roomConfigLoading=!1,this._loadVersion=0,this._touchStartX=0,this._touchDelta=0,this._isSwiping=!1,this._trackEl=null,this._cachedCameraIds=[],this._cachedCamerasKey="",this._onPointerDown=e=>{if(e.target.closest("glass-icon-button, .carousel-nav"))return;this._touchStartX=e.clientX,this._touchDelta=0,this._isSwiping=!0;e.currentTarget.setPointerCapture(e.pointerId),this._trackEl=this.shadowRoot?.querySelector(".carousel-track"),this._trackEl&&(this._trackEl.style.transition="none")},this._onPointerMove=e=>{if(!this._isSwiping)return;const t=this._trackEl??this.shadowRoot?.querySelector(".carousel-track");if(!t)return;this._trackEl=t,this._touchDelta=e.clientX-this._touchStartX;const i=e.currentTarget.offsetWidth,a=100*this._carouselIndex,s=this._touchDelta/i*100;this._trackEl.style.transform=`translateX(${-a+s}%)`},this._onPointerUp=e=>{if(!this._isSwiping||!this._trackEl)return;this._isSwiping=!1,this._trackEl.style.transition="";const t=.2*e.currentTarget.offsetWidth;this._touchDelta<-t?this._goTo(this._carouselIndex+1):this._touchDelta>t?this._goTo(this._carouselIndex-1):this._goTo(this._carouselIndex),this._trackEl=null},this._onPointerCancel=()=>{this._isSwiping&&this._trackEl&&(this._isSwiping=!1,this._trackEl.style.transition="",this._goTo(this._carouselIndex),this._trackEl=null)}}static getConfigElement(){return document.createElement("glass-camera-carousel-card-editor")}getCardSize(){return 3}connectedCallback(){super.connectedCallback(),this._listen("camera-carousel-config-changed",()=>{this._configLoaded=!1,this._cachedCamerasKey="",this._loadConfig()}),this._listen("room-config-changed",e=>{this.areaId&&e.areaId===this.areaId&&(this._roomConfig=null,this._cachedCamerasKey="",this._loadRoomConfig())}),this._listen("dashboard-config-changed",()=>this.requestUpdate()),this._timestampTimer=setInterval(()=>this.requestUpdate(),6e4)}disconnectedCallback(){super.disconnectedCallback(),this._backend=void 0,this._clearCycleTimer(),this._clearTimestampTimer(),this._clearRingTimer()}getTrackedEntityIds(){if(!this.hass)return[];const e=this.hass;return this._getCameraIds().flatMap(t=>{const i=Jr(t,e.states,e.entities);return[t,i.motionSensorId,i.recordSwitchId,i.sirenId,i.floodlightId,i.autoTrackId,i.doorbellEventId,i.visitorSensorId,i.privacySwitchId,i.detectSwitchId,i.snapshotsSwitchId,i.isDarkSensorId,i.batterySensorId,i.batteryLowSensorId,i.sleepSensorId,i.ptzUpId,i.ptzDownId,i.ptzLeftId,i.ptzRightId,i.ptzZoomInId,i.ptzZoomOutId].filter(Boolean)})}updated(e){super.updated(e),e.has("hass")&&this.hass&&(this._backend&&this._backend.connection===this.hass.connection||(this._backend=new gi(this.hass))),this.areaId!==this._lastAreaId&&(this._lastAreaId=this.areaId,this._carouselIndex=0,this._cachedCamerasKey="",this._configLoaded=!1,this._roomConfig=null,this._liveIds=new Set),this._configLoaded||this._configLoading||this._loadConfig(),!this.areaId||this._roomConfig||this._roomConfigLoading||this._loadRoomConfig()}async _loadConfig(){if(!this._backend||this._configLoading)return;this._configLoading=!0;const e=++this._loadVersion;try{const t=await this._backend.send("get_config");if(e!==this._loadVersion)return;this._camConfig=t.camera_carousel||{show_header:!0,entity_order:[],hidden_entities:[],auto_cycle:!1,cycle_interval:10},this._configLoaded=!0,this._setupCycleTimer(),this.requestUpdate()}catch{}finally{this._configLoading=!1}}async _loadRoomConfig(){if(!this._backend||!this.areaId||this._roomConfigLoading)return;this._roomConfigLoading=!0;const e=this.areaId;try{const t=await this._backend.send("get_room",{area_id:e});if(this.areaId!==e)return;this._roomConfig={hidden_entities:t?.hidden_entities??[],entity_order:t?.entity_order??[]},this._cachedCamerasKey="",this.requestUpdate()}catch{}finally{this._roomConfigLoading=!1}}_getCameraIds(){if(!this.hass)return[];let e;e=this.areaId?hi(this.areaId,this.hass.entities,this.hass.devices).filter(e=>e.entity_id.startsWith("camera.")).map(e=>e.entity_id):Object.keys(this.hass.states).filter(e=>e.startsWith("camera."));const t=new Set(this._camConfig?.hidden_entities??[]);if(this.areaId&&this._roomConfig)for(const s of this._roomConfig.hidden_entities)t.add(s);t.size&&(e=e.filter(e=>!t.has(e))),e=function(e,t){const i=e.filter(e=>!/_snapshots(_|$)/.test(e)),a=new Map,s=new Set;for(const o of i){const e=t[o]?.device_id;if(!e){s.add(o);continue}const i=a.get(e);(!i||o.length<i.length)&&a.set(e,o)}const r=new Set([...a.values(),...s]);return e.filter(e=>r.has(e))}(e,this.hass.entities);const i=e.length+":"+e.map(e=>{const t=this.hass?.states[e];return t?`${e}:${t.last_changed}`:e}).join(",");if(i===this._cachedCamerasKey)return this._cachedCameraIds;const a=this.areaId?this._roomConfig?.entity_order??[]:[];if(this.areaId&&a.length){const t=a.filter(t=>e.includes(t)),i=e.filter(e=>!t.includes(e));e=[...t,...i]}else{const t=this._camConfig?.entity_order??[];if(t.length){const i=t.filter(t=>e.includes(t)),a=e.filter(e=>!i.includes(e));if(!this.areaId){const e=this.hass.states,t=this.hass.entities;a.sort((i,a)=>this._latestAlertTimestamp(a,e,t)-this._latestAlertTimestamp(i,e,t))}e=[...i,...a]}else if(!this.areaId){const t=this.hass.states,i=this.hass.entities;e.sort((e,a)=>this._latestAlertTimestamp(a,t,i)-this._latestAlertTimestamp(e,t,i))}}return this._cachedCamerasKey=i,this._cachedCameraIds=e,this._carouselIndex>=e.length&&(this._carouselIndex=Math.max(0,e.length-1)),this._cachedCameraIds}_latestAlertTimestamp(e,t,i){const a=i[e];if(!a?.device_id)return 0;const s=a.device_id;let r=0;for(const[o,n]of Object.entries(i)){if(n.device_id!==s||!o.startsWith("binary_sensor."))continue;if(!Gr.some(([e])=>e.test(o)))continue;const e=t[o];if(!e)continue;const i=new Date(e.last_changed).getTime();i>r&&(r=i)}return r}_getCameraInfo(e){if(!this.hass)return null;const t=this.hass.states[e];if(!t)return null;const i=t.attributes?.supported_features??0,a="unavailable"!==t.state&&!1!==t.attributes?.is_on,s=Jr(e,this.hass.states,this.hass.entities),r=!(!s.doorbellEventId&&!s.visitorSensorId),o=r?function(e,t,i,a){if(e){const t=i[e];if(t&&"ring"===t.attributes?.event_type&&t.state&&"unknown"!==t.state&&"unavailable"!==t.state){const e=new Date(t.state).getTime();if(Number.isFinite(e)){const t=3e4-Math.max(0,a-e);if(t>0)return t}}}if(t){const e=i[t];if("on"===e?.state){const t=new Date(e.last_changed).getTime();if(Number.isFinite(t)){const e=3e4-Math.max(0,a-t);if(e>0)return e}}}return 0}(s.doorbellEventId,s.visitorSensorId,this.hass.states,Date.now()):0,n="on"===(s.privacySwitchId?this.hass.states[s.privacySwitchId]?.state:void 0);return{entityId:e,entity:t,name:t.attributes?.friendly_name||e.split(".")[1],state:t.state,isOn:a,features:i,entityPicture:t.attributes?.entity_picture??null,motionSensorId:s.motionSensorId,motionDetectionSupported:void 0!==t.attributes?.motion_detection,motionDetectionEnabled:!0===t.attributes?.motion_detection,hasMotion:!!s.motionSensorId&&"on"===this.hass.states[s.motionSensorId]?.state,recordSwitchId:s.recordSwitchId,isRecording:"recording"===t.state||!!s.recordSwitchId&&"on"===this.hass.states[s.recordSwitchId]?.state,sirenId:s.sirenId,floodlightId:s.floodlightId,autoTrackId:s.autoTrackId,aiDetected:s.aiDetected,icon:Zr(t,r),isDoorbell:r,doorbellEventId:s.doorbellEventId,visitorSensorId:s.visitorSensorId,isRinging:o>0,ringExpiresInMs:o,privacySwitchId:s.privacySwitchId,isPrivacyOn:n,detectSwitchId:s.detectSwitchId,isDetectOn:!!s.detectSwitchId&&"on"===this.hass.states[s.detectSwitchId]?.state,snapshotsSwitchId:s.snapshotsSwitchId,isSnapshotsOn:!!s.snapshotsSwitchId&&"on"===this.hass.states[s.snapshotsSwitchId]?.state,isDarkSensorId:s.isDarkSensorId,isDark:!!s.isDarkSensorId&&"on"===this.hass.states[s.isDarkSensorId]?.state,batterySensorId:s.batterySensorId,batteryLevel:this._readBatteryLevel(s.batterySensorId),isBatteryLow:this._readBatteryLow(s.batteryLowSensorId,s.batterySensorId),sleepSensorId:s.sleepSensorId,isSleeping:!!s.sleepSensorId&&"on"===this.hass.states[s.sleepSensorId]?.state,hasPtz:!!(s.ptzUpId||s.ptzDownId||s.ptzLeftId||s.ptzRightId),ptzUpId:s.ptzUpId,ptzDownId:s.ptzDownId,ptzLeftId:s.ptzLeftId,ptzRightId:s.ptzRightId,ptzZoomInId:s.ptzZoomInId,ptzZoomOutId:s.ptzZoomOutId}}_batteryIcon(e){if(e<=10)return"mdi:battery-alert-variant-outline";if(e>=95)return"mdi:battery";return`mdi:battery-${Math.max(10,Math.min(90,10*Math.floor(e/10)))}`}_readBatteryLevel(e){if(!e||!this.hass)return null;const t=this.hass.states[e]?.state;if(!t||"unknown"===t||"unavailable"===t)return null;const i=parseFloat(t);return Number.isFinite(i)?Math.max(0,Math.min(100,Math.round(i))):null}_readBatteryLow(e,t){if(e&&"on"===this.hass?.states[e]?.state)return!0;const i=this._readBatteryLevel(t);return null!==i&&i<20}_setupCycleTimer(){if(this._clearCycleTimer(),this._camConfig?.auto_cycle&&this._getCameraIds().length>1){const e=1e3*(this._camConfig.cycle_interval||10);this._cycleTimer=setInterval(()=>{if(this._isSwiping)return;const e=this._getCameraIds();e.length>1&&(this._carouselIndex=(this._carouselIndex+1)%e.length,this.requestUpdate())},e)}}_clearCycleTimer(){this._cycleTimer&&(clearInterval(this._cycleTimer),this._cycleTimer=void 0)}_clearTimestampTimer(){this._timestampTimer&&(clearInterval(this._timestampTimer),this._timestampTimer=void 0)}_clearRingTimer(){this._ringTimer&&(clearTimeout(this._ringTimer),this._ringTimer=void 0)}_scheduleRingFadeOut(e){this._ringTimer&&clearTimeout(this._ringTimer),this._ringTimer=setTimeout(()=>{this._ringTimer=void 0,this.requestUpdate()},Math.max(200,e+200))}_goTo(e){const t=this._getCameraIds();t.length&&(this._carouselIndex=(e%t.length+t.length)%t.length,this._foldOpen=!1,this._setupCycleTimer(),this.requestUpdate())}_prev(){this._goTo(this._carouselIndex-1)}_next(){this._goTo(this._carouselIndex+1)}_togglePower(e){if(!this.hass)return;const t=e.isOn?"turn_off":"turn_on";this._safeCallService("camera",t,{entity_id:e.entityId})}_snapshot(e){if(!this.hass)return;const t=new CustomEvent("hass-more-info",{detail:{entityId:e.entityId},bubbles:!0,composed:!0});this.dispatchEvent(t)}_toggleRecord(e){if(!this.hass||!e.recordSwitchId)return;const t="on"===this.hass.states[e.recordSwitchId]?.state;this._safeCallService("switch",t?"turn_off":"turn_on",{entity_id:e.recordSwitchId})}_toggleMotion(e){if(!this.hass)return;const t=e.motionDetectionEnabled?"disable_motion_detection":"enable_motion_detection";this._safeCallService("camera",t,{entity_id:e.entityId})}_toggleSiren(e){if(!this.hass||!e.sirenId)return;const t="on"===this.hass.states[e.sirenId]?.state;this._safeCallService("siren",t?"turn_off":"turn_on",{entity_id:e.sirenId})}_toggleFloodlight(e){if(!this.hass||!e.floodlightId)return;const t="on"===this.hass.states[e.floodlightId]?.state;this._safeCallService("light",t?"turn_off":"turn_on",{entity_id:e.floodlightId})}_toggleAutoTrack(e){if(!this.hass||!e.autoTrackId)return;const t="on"===this.hass.states[e.autoTrackId]?.state;this._safeCallService("switch",t?"turn_off":"turn_on",{entity_id:e.autoTrackId})}_togglePrivacy(e){this.hass&&e.privacySwitchId&&this._safeCallService("switch",e.isPrivacyOn?"turn_off":"turn_on",{entity_id:e.privacySwitchId})}_toggleDetect(e){this.hass&&e.detectSwitchId&&this._safeCallService("switch",e.isDetectOn?"turn_off":"turn_on",{entity_id:e.detectSwitchId})}_toggleSnapshots(e){this.hass&&e.snapshotsSwitchId&&this._safeCallService("switch",e.isSnapshotsOn?"turn_off":"turn_on",{entity_id:e.snapshotsSwitchId})}_ptzPress(e){this.hass&&e&&this._safeCallService("button","press",{entity_id:e})}_toggleFullscreen(){const e=this.shadowRoot?.querySelector(".carousel-hero");e&&(document.fullscreenElement===e?document.exitFullscreen().catch(()=>{}):e.requestFullscreen?.().catch(()=>{}))}_startStream(e){const t=new Set(this._liveIds);t.add(e),this._liveIds=t}render(){if(this._lang,!this.hass)return W;const e=this._getCameraIds();if(!e.length)return W;const t=!1!==this._camConfig?.show_header,i=this._getCameraInfo(e[this._carouselIndex]);let a=1/0;for(const r of e){const e=this._getCameraInfo(r);e?.isRinging&&e.ringExpiresInMs<a&&(a=e.ringExpiresInMs)}a!==1/0?this._scheduleRingFadeOut(a):this._ringTimer&&this._clearRingTimer();const s=this._bindGesture({onTap:()=>{const t=e[this._carouselIndex];t&&!this._liveIds.has(t)&&this._startStream(t)},onLongPress:()=>{this._isSwiping=!1,this._trackEl=null,this._foldOpen=!this._foldOpen},exclude:"glass-icon-button"});return N`
      ...
      <div class="cam-wrap ${this._foldOpen?"fold-open":""} ${this._heroPulseClass(i)}">
        <div class="carousel-hero ${i?.isDoorbell?"aspect-portrait":""} ${this.previewFullscreen?"fs-preview":""}"
    ...
    `}_renderActions(e){if(!e.isOn)return N`
        <div class="carousel-actions">
          ...
          >${ei("camera.power_on")}</glass-button>
        </div>
      ...
      <div class="carousel-actions">
        ...
        <glass-button
          ...
          @click=${()=>this._snapshot(e)}
        >${ei("camera.snapshot")}</glass-button>
        ${e.recordSwitchId?N`
          <glass-button
            size="sm"
            variant="ghost"
          ...
          >${e.isRecording?ei("camera.record_stop"):ei("camera.record_start")}</glass-button>
        `:W}
        ${e.motionDetectionSupported?N`
          <glass-icon-button
            size="md"
