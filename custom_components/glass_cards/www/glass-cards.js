...
`;var ws=Object.defineProperty,xs=(e,t,i,a)=>{for(var r,s=void 0,o=e.length-1;o>=0;o--)(r=e[o])&&(s=r(t,i,s)||s);return s&&ws(t,i,s),s};const ks={perso:{color:"var(--c-accent)",label:"Personnel"},travail:{color:"var(--c-info)",label:"Travail"},famille:{color:"#f472b6",label:"Famille"},taches:{color:"var(--c-warning)",label:"Tâches"},anniversaires:{color:"var(--c-success)",label:"Anniversaires"}},$s=["Dim","Lun","Mar","Mer","Jeu","Ven","Sam"],Cs=["janv.","févr.","mars","avr.","mai","juin","juil.","août","sept.","oct.","nov.","déc."];function Ss(e){return 0===(e.dayOffset??0)&&!(!e.now&&!e.allday&&e.time&&!function(e){const t=e.split(" - "),i=t[0],a=t.length>1?t[1]:t[0],[r,s]=a.split(":").map(e=>parseInt(e,10)),[o,n]=i.split(":").map(e=>parseInt(e,10)),l=new Date,c=new Date;return c.setHours(r,s,0,0),(r<o||r===o&&s<n)&&c.setDate(c.getDate()+1),c>l}(e.time))}class zs extends Oi{constructor(){super(...arguments),this.events=[],this.showHeader=!0,this._open=!1,this._selectedDayOffset=0,this._tickerIdx=0,this._tickerLeavingIdx=null,this._hiddenEntities=[],this._fetchedEvents=[],this._configLoaded=!1,this._configLoadInFlight=!1,this._fetchInFlight=!1,this._lastEventsKey="",this._lastTodayKey="",this._foldId=`cal-fold-${Math.random().toString(36).slice(2,9)}`}getCardSize(){return 1}connectedCallback(){super.connectedCallback(),this._startTicker(),this._listen("calendar-config-changed",()=>{this._configLoaded=!1,this._loadConfigAndFetch()})}disconnectedCallback(){super.disconnectedCallback(),this._stopTicker(),this._stopRefreshTimer(),this._stopMidnightWatcher(),this._backend=void 0,this._configLoaded=!1}willUpdate(e){super.willUpdate(e),e.has("hass")&&this.hass&&!this.configPreview&&(this._backend&&this._backend.connection!==this.hass.connection&&(this._backend=void 0,this._configLoaded=!1,this._configLoadInFlight=!1),this._configLoaded||this._loadConfigAndFetch())}async _loadConfigAndFetch(){if(this.hass&&!this._configLoadInFlight){this._configLoadInFlight=!0;try{this._backend||(this._backend=new Hi(this.hass));try{const e=await this._backend.send("get_config");this._hiddenEntities=e?.calendar_card?.hidden_entities??[],void 0!==e?.calendar_card?.show_header&&(this.showHeader=e.calendar_card.show_header),this._configLoaded=!0}catch{this._configLoaded=!0}await this._fetchEvents(),this._startRefreshTimer(),this._startMidnightWatcher()}finally{this._configLoadInFlight=!1}}}_startRefreshTimer(){this._stopRefreshTimer(),this._refreshTimer=setInterval(()=>{this._fetchEvents()},9e5)}_stopRefreshTimer(){this._refreshTimer&&(clearInterval(this._refreshTimer),this._refreshTimer=void 0)}_todayKey(){const e=new Date;return`${e.getFullYear()}-${e.getMonth()}-${e.getDate()}`}_startMidnightWatcher(){this._stopMidnightWatcher(),this._lastTodayKey=this._todayKey(),this._midnightTimer=setInterval(()=>{const e=this._todayKey();e!==this._lastTodayKey&&(this._lastTodayKey=e,this._selectedDayOffset=0,this._fetchEvents())},6e4)}_stopMidnightWatcher(){this._midnightTimer&&(clearInterval(this._midnightTimer),this._midnightTimer=void 0)}async _fetchEvents(){if(this.hass&&!this.configPreview&&!this._fetchInFlight){this._fetchInFlight=!0;try{const e=new Set(this._hiddenEntities),t=Object.keys(this.hass.states).filter(t=>t.startsWith("calendar.")&&!e.has(t));if(0===t.length)return void(this._fetchedEvents=[]);const i=new Date;i.setHours(0,0,0,0);const a=new Date(i);a.setDate(a.getDate()+7);const r=i.toISOString(),s=a.toISOString(),o=`start=${encodeURIComponent(r)}&end=${encodeURIComponent(s)}`,n=(await Promise.all(t.map(async e=>{try{const t=await this.hass.callApi("GET",`calendars/${e}?${o}`);return(Array.isArray(t)?t:[]).flatMap(t=>this._toCardEvents(t,e))}catch(t){return console.warn(`[glass-calendar-card] failed to fetch events for ${e}`,t),[]}}))).flat();n.sort((e,t)=>(e.allday?0:1)-(t.allday?0:1)||(e.time??"").localeCompare(t.time??"")),this._fetchedEvents=n}finally{this._fetchInFlight=!1}}}_toCardEvents(e,t){const i=t.split(".")[1]||"perso",a=new Date;if(a.setHours(0,0,0,0),e.start.date&&!e.start.dateTime){const t=new Date(e.start.date+"T00:00:00"),r=e.end.date?new Date(e.end.date+"T00:00:00"):new Date(t.getTime()+864e5),s=Math.round((t.getTime()-a.getTime())/864e5),o=Math.round((r.getTime()-a.getTime())/864e5),n=Math.max(0,s),l=Math.min(7,o),c=[];for(let a=n;a<l;a++)c.push({title:e.summary,time:null,cal:i,allday:!0,dayOffset:a});return c}const r=e.start.dateTime??"",s=e.end.dateTime??"",o=r?new Date(r):null,n=s?new Date(s):null,l=e=>e?`${e.getHours().toString().padStart(2,"0")}:${e.getMinutes().toString().padStart(2,"0")}`:"",c=l(o),d=l(n),h=d&&d!==c?`${c} - ${d}`:c||null,p=new Date,u=!!(o&&n&&o<=p&&n>=p);let g=0;if(o){const e=new Date(o);e.setHours(0,0,0,0),g=Math.round((e.getTime()-a.getTime())/864e5)}return g<0||g>6?[]:[{title:e.summary,time:h,cal:i,now:u,dayOffset:g}]}updated(e){if(super.updated(e),e.has("events")||e.has("_fetchedEvents")){const e=this._eventsKey(this._allEvents());e!==this._lastEventsKey&&(this._lastEventsKey=e,this._stopTicker(),this._tickerIdx=0,this._tickerLeavingIdx=null,this._startTicker())}else!this._tickerTimer&&this._tickerEvents().length>1&&this._startTicker()}_eventsKey(e){return e.map(e=>`${e.title}${e.time??""}${e.dayOffset??""}${e.allday?1:0}${e.now?1:0}`).join("")}_allEvents(){return this.events.length>0?this.events:this._fetchedEvents}_collapseExpanded(){this._open&&(this._open=!1)}_tickerEvents(){return this._allEvents().filter(Ss)}_startTicker(){this._tickerEvents().length<=1||window.matchMedia("(prefers-reduced-motion: reduce)").matches||(this._tickerTimer=setInterval(()=>this._advanceTicker(),3500))}_stopTicker(){this._tickerTimer&&clearInterval(this._tickerTimer),this._tickerTimer=void 0,this._leaveTimer&&clearTimeout(this._leaveTimer),this._leaveTimer=void 0}_advanceTicker(){const e=this._tickerEvents();if(e.length<=1)return;const t=this._tickerIdx;this._tickerLeavingIdx=t,this._tickerIdx=(t+1)%e.length,this._leaveTimer=setTimeout(()=>{this._tickerLeavingIdx=null},500)}_toggleOpen(){this._open=!this._open}render(){const e=new Date,t=`${$s[e.getDay()]} ${e.getDate()} ${Cs[e.getMonth()]}`,i=this._tickerEvents(),a=i.filter(e=>e.now).length;return te`
      ...
      <div class="rooms-section">
        <div class="rooms-scroller">
          <div class="rooms-track" role="group" aria-label="Pièces à nettoyer">
            ${e.roomButtons.map(({entityId:e,slug:t})=>{const i=Fs(t),a=this._isCurrentRoomButton(e,t);return te`
                <glass-chip
                  size="sm"
                  active-color="cool"
                  ?active=${a}
    ...
    `}_renderLavage(e){if(!e)return re;if(!(e.mopIntensity||e.mopPattern||e.mopAttached))return re;const t=this.hass.states[e.mopIntensity??""],i=this.hass.states[e.mopPattern??""],a=t?.attributes.options??[],r=i?.attributes.options??[],s=t?.state,o=i?.state;return te`
      <div class="fold-section">
        ...
        <div class="status-row">
          ${this._renderBadge(Ps(this.hass,e.mopAttached)?{label:gi("vacuum.mop_attached"),variant:"success",icon:"mdi:check-circle"}:{label:gi("vacuum.mop_missing"),variant:"alert",icon:"mdi:alert-circle-outline"})}
          ${this._renderBadge(Ps(this.hass,e.tankAttached)?{label:gi("vacuum.tank_ok"),variant:"success",icon:"mdi:check-circle"}:{label:gi("vacuum.tank_missing"),variant:"alert",icon:"mdi:alert-circle-outline"})}
          ${this._renderBadge(Ps(this.hass,e.waterShortage)?{label:gi("vacuum.water_short"),variant:"alert",icon:"mdi:water-off"}:{label:gi("vacuum.water_ok"),variant:"success",icon:"mdi:water"})}
        </div>
      </div>
    ...
    `}_renderDock(e){if(!e)return re;const t=[{icon:"mdi:battery-charging",label:Ps(this.hass,e.charging)?gi("vacuum.dock_charging"):gi("vacuum.dock_idle"),variant:Ps(this.hass,e.charging)?"success":"idle"},{icon:"mdi:hair-dryer-outline",label:Ps(this.hass,e.dockDrying)?gi("vacuum.dock_drying_label",{minutes:Math.round(Ms(this.hass,e.dockDryingTimeLeft,0))}):gi("vacuum.dock_drying_idle"),variant:Ps(this.hass,e.dockDrying)?"info":"idle"},{icon:"mdi:water-pump",label:Ps(this.hass,e.dirtyWaterBox)?gi("vacuum.dirty_full"):gi("vacuum.dirty_ok"),variant:Ps(this.hass,e.dirtyWaterBox)?"alert":"success"},{icon:"mdi:water",label:Ps(this.hass,e.cleanWaterBox)?gi("vacuum.clean_ok"):gi("vacuum.clean_empty"),variant:Ps(this.hass,e.cleanWaterBox)?"success":"alert"},{icon:"mdi:bottle-tonic-outline",label:Ps(this.hass,e.cleaningFluid)?gi("vacuum.fluid_ok"):gi("vacuum.fluid_empty"),variant:Ps(this.hass,e.cleaningFluid)?"success":"warning"}],i={success:"var(--c-success)",alert:"var(--c-alert)",warning:"var(--c-warning)",info:"var(--c-info)",idle:"var(--t3)"};return te`
      <div class="fold-section">
    ...
    `}_renderConso(e){if(!e)return re;const t=[{key:e.consoBrushMain,label:gi("vacuum.conso_brush_main"),max:300},{key:e.consoBrushSide,label:gi("vacuum.conso_brush_side"),max:200},{key:e.consoFilter,label:gi("vacuum.conso_filter"),max:150},{key:e.consoSensors,label:gi("vacuum.conso_sensors"),max:100}].filter(e=>e.key);return 0===t.length?re:te`
      <div class="fold-section">
        ...
        ${t.map(e=>{const t=Ms(this.hass,e.key,0);let i="success";t<20?i="alert":t<50&&(i="warning");const a=Math.max(0,Math.min(100,t/e.max*100)),r=t<0?gi("vacuum.conso_clean_now"):gi("vacuum.conso_hours",{hours:Math.round(t)});return te`
            <div class="conso-row">
    ...
    `}_renderStats(e){if(!e)return re;const t=Ls(this.hass,e.lastEnd,""),i=Ms(this.hass,e.durationCurrent,0),a=Ms(this.hass,e.areaCurrent,0),r=Math.round(Ms(this.hass,e.totalCleanings,0)),s=Math.round(Ms(this.hass,e.areaTotal,0)),o=t?function(e){if(!e)return"";const t=new Date(e);if(isNaN(t.getTime()))return"";const i=Math.floor((Date.now()-t.getTime())/1e3);return i<60?gi("vacuum.time_just_now"):i<3600?gi("vacuum.time_minutes_ago",{n:Math.floor(i/60)}):i<86400?gi("vacuum.time_hours_ago",{n:Math.floor(i/3600)}):i<172800?gi("vacuum.time_yesterday"):gi("vacuum.time_days_ago",{n:Math.floor(i/86400)})}(t):"—",n=`${Math.round(i)} min`,l=`${a} m²`;return te`
      <div class="fold-section">
        <glass-section-title label=${gi("vacuum.section_stats")}></glass-section-title>
        <div class="stats-row">${gi("vacuum.stats_last_session",{when:o,duration:n,area:l})}</div>
        <div class="stats-row stats-totals">
          ${gi("vacuum.stats_totals",{count:r,area:`${s} m²`})}
        </div>
      </div>
    `}}Bs([Te({attribute:!1})],Ws.prototype,"hass"),Bs([Te({attribute:!1})],Ws.prototype,"config"),Bs([Ee()],Ws.prototype,"_open"),Bs([Ee()],Ws.prototype,"_pendingAction"),Bs([Ee()],Ws.prototype,"_locateFlashing"),Bs([Ee()],Ws.prototype,"_optimisticRoom"),Bs([Ee()],Ws.prototype,"_showHeader"),Bs([Ee()],Ws.prototype,"_configEntity"),customElements.define("glass-vacuum-card",Ws),function(){if(a)return;a=!0;const e=history.pushState,t=history.replaceState;history.pushState=function(t,a,s){if(e.call(this,t,a,s),!r){r=!0;try{window.dispatchEvent(new Event("location-changed")),i.emit("location-changed",void 0)}finally{r=!1}}},history.replaceState=function(e,a,s){if(t.call(this,e,a,s),!r){r=!0;try{window.dispatchEvent(new Event("location-changed")),i.emit("location-changed",void 0)}finally{r=!1}}},window.addEventListener("popstate",s)}(),ti||(ti=new ei)}();
