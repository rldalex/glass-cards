...
import { t, getLanguage } from '@glass-cards/i18n';
...
function windBearingToDir(bearing: number): string {
...
export class ConfigTabWeather extends BaseConfigTab {
  ...
  renderPreview(): TemplateResult | typeof nothing {
    ...
    const dateStr = now.toLocaleDateString(this.hass.language || getLanguage(), { weekday: 'long', day: 'numeric', month: 'long' });
    ...
    if (!hidden.has('wind') && attrs.wind_speed != null) {
      const dir = typeof attrs.wind_bearing === 'number' ? windBearingToDir(attrs.wind_bearing as number) : undefined;
    ...
    // Forecast
    const lang = this.hass.language || getLanguage();
